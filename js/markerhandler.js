...
AFRAME.registerComponent("markerhandler", {
    ...
    if (elementsArray.length > 1) {
      ...
      if (length === 3) {
        ...
        if (distance1<1.25 && distance2<1.25){
          if (compound !== undefined) {
            var barcodeValue1 = elementsArray[0].barcode_value

            this.showCompound(compound,barcodeValue1);
          } else {
            messageText.setAttribute("visible", true);
          }
        } else {
      ...
      if (A.includes(el.element_name)) {
        ...
        for (var i of elementsArray) {
          ...
          if (C.includes(i.element_name)){
            var count= this.counteroccurens(elementsArray,el.element_name)
            if(count > 1 ){
  ...
  counteroccurens: function (arr,y){
    return arr.reduce((a,v)=>(v.element_name === y ? a + 1 : a ) , 0 );
  }
});
